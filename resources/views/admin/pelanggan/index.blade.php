...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
	<div class="d-block mb-4 mb-md-0">
		...
		<h2 class="h4">FOR PELANGGAN</h2>
		<p class="mb-0">List Pelanggan</p>
	</div>
	<div class="btn-toolbar mb-2 mb-md-0">
		<a href="{{route('pelanggan.create')}}" class="btn btn-sm btn-success d-inline-flex align-items-center">
	...
	<div class="table-responsive">
		<table class="table table-striped table-hover">
			...
			<tbody>
				...
				$no=0;
				@endphp
				@foreach($data_pelanggan as $b)
				<tr>
					<td>{{($data_pelanggan->currentPage() - 1) * $data_pelanggan->perPage() + $loop->iteration}}</td>
					<td>{{ $b->first_name }}</td>
					<td>{{ $b->last_name }}</td>
					<td>{{ $b->birthday }}</td>
					<td>{{ $b->gender }}</td>
					...
					<td>
						...
						<a href="{{ route ('pelanggan.edit', $b->pelanggan_id) }}" class="btn btn-warning btn-sm" title="Edit">
						...
						<!-- Tombol Delete -->
						<form action="{{ route('pelanggan.destroy', $b->pelanggan_id) }}" method="POST" style="display:inline;">
							@csrf
							@method('DELETE')
